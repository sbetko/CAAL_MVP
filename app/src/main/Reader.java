...
public class Reader extends Service implements SensorEventListener {
    // global prefs
    static int maxDataPoints = 250;
    static int curDataPoints = 0;
    static int sensorRate = 50;
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (mySensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            if ((curTime - lastUpdate) > sensorRate) {
                ...
                System.out.println(curDataPoints);
                    // if its time to stop reading sensor data
                    if (curDataPoints >= maxDataPoints) {
                        System.out.println("finished collecting raw data"); // DEBUG
